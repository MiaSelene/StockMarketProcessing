...
def moddedOST(List):
    ...
    for i in range(stop,len(List)):
        Entry=List[i]
        Needed=Estimate*(1-(i-stop)/(len(List)-stop))
        if Entry>Needed:
            return Entry,i
    return Entry,i
...
while i<(len(Werte)-Periode):
    BuyPreis,BuyPunkt=moddedOST(numpy.multiply(Werte[i:i+Periode],-1))
    BuyPreis*=-1
    i+=BuyPunkt
...
plt.plot(numpy.fft.ifft([0 if abs(x)<10000 else x for x in numpy.fft.rfft(Werte)]))

plt.subplot(2,2,4)
plt.plot([B-S for B,S in zip(YB,YS)])
